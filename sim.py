...
for prety in ['EV1', 'EV1DE', 'EV1DNE', 'EV1DN', 'EV1DNW', 'EV1DW','EV1DSW', 'EV1DS','EV1DSE', 'EV4', 'EMT']:
  for posty in ['EMR', 'EML']:
    for strty,synmech,weight,plastty in zip(['','n'],['AMPA', 'NMDA'],[0.025*cfg.EEGain, 0.02*cfg.EEGain],[STDPparamsRL1,STDPparamsRL2]):
...
def trainAgentFake(t):
    ...
    if t<21.0: # for the first time interval use first action randomly and other four actions based on relative position of ball and agent.
        if sim.rank == 0:
            rewards, actions, epCount, InputImages = sim.AIGame.playGameFake(epCount, InputImages)
