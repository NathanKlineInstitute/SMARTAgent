...
#define nrn_init _nrn_init__hsyn
#define _nrn_initial _nrn_initial__hsyn
#define nrn_cur _nrn_cur__hsyn
#define _nrn_current _nrn_current__hsyn
#define nrn_jacob _nrn_jacob__hsyn
#define nrn_state _nrn_state__hsyn
#define _net_receive _net_receive__hsyn 
#define resetscaling resetscaling__hsyn 
#define state state__hsyn 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 hsyn /Users/anwarharoon/Documents/NKI-modeling/SMARTAgent/x86_64/hsyn.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 if ( verbose ) {
   ...
   if ( _args[0] < 0.0 ) {
    ...
    double __state = firingrate;
    double __primary = (firingrate + rateinc) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / ratedectau ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / ratedectau ) - __primary );
    firingrate += __primary;
  } else {
 firingrate = firingrate + rateinc ;
    ...
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = A;
    double __primary = (A + _args[0] * factor * scalefactor) - __state;
 ...
 A = A + _args[0] * factor * scalefactor ;
   ...
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B;
    double __primary = (B + _args[0] * factor * scalefactor) - __state;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/anwarharoon/Documents/NKI-modeling/SMARTAgent/hsyn.mod";
static const char* nmodl_file_text = 
  "NEURON {\n"
  "  POINT_PROCESS hsyn\n"
