...
PROCEDURE reward_punish (reinf) {
  ...
  if (RLon == 1 && fabs(reinf)>0.0) { : If RL is turned on...
    ...
    :}
    :if (fabs(deltaw)>0.0){
     cumdeltaw = cumdeltaw + deltaw	     
     : adjustweight(deltaw) : Adjust the weight.			      
    : cumreward = cumreward + reinf
...
PROCEDURE setweight (wt) {
   synweight = wt
}

PROCEDURE saveweight () {
   tmpweight = synweight
   tmpcumdeltaw = cumdeltaw					   
}

PROCEDURE restoreweight () {
   synweight = tmpweight
   cumdeltaw = tmpcumdeltaw					      
}
			   
PROCEDURE settestweight () {
   saveweight()
   adjustweight(cumdeltaw)
}
			 
PROCEDURE applycumdeltaw (deltareward) {
   restoreweight()						      
   adjustweight(cumdeltaw * deltareward)
   cumdeltaw = 0.0		
}
