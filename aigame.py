    ...
    def playGame (self, actions, epCount, InputImages, last_obs, last_ball_dir): #actions need to be generated from motor cortex
      ...
      for a in range(self.intaction):
        ...
        if np.shape(last_obs)[0]>0: #if last_obs is not empty              
          xpos_Ball, ypos_Ball = findobj(last_obs, courtXRng, courtYRng) # get x,y positions of ball
          xpos_Racket, ypos_Racket = findobj(last_obs, racketXRng, courtYRng) # get x,y positions of racket
        ...
        self.env.render()
        last_obs = observation #current observation will be used as last_obs for the next action; does this get saved properly...?
        if done:
          self.env.reset()
