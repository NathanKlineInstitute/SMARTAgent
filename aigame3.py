"""
AIGame: connects OpenAI gym game to the model (V1-M1-RL) # Pong-specific
Adapted from arm.py
...
--------------------------
AIGame2: connects OpenAI gym game to the model (V1-M1-RL) for Breakout environment
Adapted from AIGame.py
Original Version: 2020jul7 by davidd
--------------------------
AIGame3: connects OpenAI gym game to the model (V1-M1-RL) # Universal/Combo of Breakout + Pong
Adapted from AIGame.py and AIGame2.py
Original Version: 2020aug4 by davidd
"""
#Original imports from aigame.py
...
try:
  ...
  if useSimulatedEnv:
    from simulateBreakout import simulateBreakout	#simulatedBreakout doesn't exist yet, but in case it becomes necessary
    breakout = simulateBreakout()
  else:
...
class AIGame:
  ...
  def __init__ (self,fcfg='sim.json'): # initialize variables
    if useSimulatedEnv: self.breakout = breakout
    else: self.env = env
    self.countAll = 0
  ...
  def predictBallRacketXIntercept(self, xpos1, ypos1, xpos2, ypos2): # Vertical games like breakout
    courtYRng, courtXRng = self.courtYRng, self.courtXRng # coordinate ranges for different objects    
  ...
  def predictBallRacketYIntercept(self, xpos1, ypos1, xpos2, ypos2): # Horizontal games like pong
    courtYRng, courtXRng = self.courtYRng, self.courtXRng # coordinate ranges for different objects    
    courtHeight = courtYRng[1] - courtYRng[0]
    courtWidth = courtXRng[1] - courtXRng[0]
    if ((xpos1==-1) or (xpos2==-1)):
      ...
      if deltax<=0:
        ...
        if ypos1<0:
          ...
          NB_intercept_steps = np.ceil((CourtHeight - xpos2)/deltax)
          deltay = ypos2-ypos1
          predY_nodeflection = ypos2 + (NB_intercept_steps*deltay)
          if predY_nodeflection<0:
            predY = -1*predY_nodeflection
          elif predY_nodeflection>courtWidth:
            predY = predY_nodeflection-courtWidth
          else:
